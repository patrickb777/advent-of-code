...
// Test expanded seed list
func TestExpandSeedList(t *testing.T) {
	seeds := []int{79, 14, 55, 13}
	expectedResult := 27
	seedList := expSeedList(seeds)
	if len(seedList) != expectedResult {
		t.Errorf("Almanac parsing did not return correct number of seeds, got: %v, want: %v.", len(seedList), expectedResult)
	}
}
